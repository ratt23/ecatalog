      ...
      if ((path === "/catalog-items" || path === "/catalog-items/") && method === "PUT") {
        ...
        const [updated] = await sql`
          UPDATE catalog_items SET 
            ...
            sort_order=${sort_order !== void 0 ? sort_order : 0},
      ...
      if (path === "/catalog-items/reorder" && method === "POST") {
        ...
        await sql.begin(async (sql2) => {
          for (let i = 0; i < orderedIds.length; i++) {
            await sql2`UPDATE catalog_items SET sort_order = ${i} WHERE id = ${orderedIds[i]}`;
          }
        });
    ...
    if (path === "/catalog/reorder" && method === "POST") {
      ...
      await sql.begin(async (sql2) => {
        for (const item of items) {
          await sql2`UPDATE catalog_items SET sort_order = ${item.sort_order} WHERE id = ${item.id}`;
        }
      });
